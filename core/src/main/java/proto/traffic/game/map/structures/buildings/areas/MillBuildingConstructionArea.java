...
public class MillBuildingConstructionArea extends BuildingConstructionArea {
    public MillBuildingConstructionArea (MapGraph mapGraph) {
        super(mapGraph);

        float x = -mapGraph.getWidth()/2;
        float y = -mapGraph.getHeight()/2;
        float len = mapGraph.getHeight()/4;
        Rectangle firstRectangle = new Rectangle(x + len*3, y + len*2, len*2, len*2);
        Rectangle secondRectangle = new Rectangle(x + len*1, y, len*2, len*2);
