...
public class DoublePathNodeBatch extends PathNodeBatch {
    public DoublePathNodeBatch (Vector3 position, PathGraph pathGraph) {
        ...
        Vector2 leftPoint = new Vector2(position.x - Constants.roadRadius*0.75f, position.z);
        Vector2 rightPoint = new Vector2(position.x + Constants.roadRadius*0.75f, position.z);
        ...
        pathGraph.connectNodes(right30Node, right90Node);
        pathGraph.connectNodes(right90Node, right150Node);
        pathGraph.connectNodes(right150Node, left30Node);
        pathGraph.connectNodes(left30Node, left90Node);
        pathGraph.connectNodes(left90Node, left150Node);
        pathGraph.connectNodes(left150Node, right30Node);
    }
}
