...
public class WoodBuildingConstructionArea extends BuildingConstructionArea {
    public WoodBuildingConstructionArea (MapGraph mapGraph) {
        super(mapGraph);

        float x = -mapGraph.getWidth()/2;
        float y = -mapGraph.getHeight()/2;
        float len = mapGraph.getHeight()/4;
        Rectangle firstRectangle = new Rectangle(x, y, len, len*4);
        Rectangle secondRectangle = new Rectangle(x + len*5, y, len, len*4);
